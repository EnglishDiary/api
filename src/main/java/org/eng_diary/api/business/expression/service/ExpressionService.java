...
import org.eng_diary.api.domain.member.dto.response.UserDTO;
...
//        member.setId(memberId);
    ...
    public Page<ExpressionDTO> getExpressionList(Pageable pageable) {
        List<Expression> expressions = expressionRepository.getExpressionList(pageable);
        JPAQuery<Long> countQuery = expressionRepository.getExpressionCountQuery();
        ...
        List<ExpressionDTO> expressionList = expressions.stream().map((exp) -> {
            ...
            userInfo.setName(member.getNickname());
